...
function RegionsContent({ initialRegions, initialTotal }: RegionsPageClientProps) {
    const router = useRouter();
    const searchParams = useSearchParams();
    const searchQuery = searchParams.get('q');
    
    const [regions, setRegions] = useState<Region[]>(initialRegions);
    const [loading, setLoading] = useState(initialRegions.length === 0);
    const [error, setError] = useState<string | null>(null);
    const [showAll, setShowAll] = useState(false);
    const [totalRegions, setTotalRegions] = useState(initialTotal);

    useEffect(() => {
        if (!showAll && !searchQuery && initialRegions.length > 0) {
            setRegions(initialRegions);
            setTotalRegions(initialTotal);
            setLoading(false);
            return;
        }
        ...
        fetchRegions();
    }, [showAll, searchQuery, initialRegions, initialTotal]);
...
export default function RegionsPageClient({ initialRegions, initialTotal }: RegionsPageClientProps) {
    return (
        <LandingPageLayout>
            <Suspense fallback={
                ...
                <RegionsContent initialRegions={initialRegions} initialTotal={initialTotal} />
            </Suspense>
        </LandingPageLayout>
    );
}
